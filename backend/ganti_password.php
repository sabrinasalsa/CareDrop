...
if ($_SERVER['REQUEST_METHOD'] !== 'POST') { echo json_encode(['ok'=>false,'error'=>'Invalid']); exit; }
...
if (strlen($pass_baru) < 6)        { echo json_encode(['ok'=>false,'error'=>'Password baru minimal 6 karakter']); exit; }
if ($pass_baru !== $pass_baru2)    { echo json_encode(['ok'=>false,'error'=>'Konfirmasi password tidak cocok']); exit; }
...
try {
    ...
    if (!$row || !password_verify($pass_lama, $row['password'])) {
        echo json_encode(['ok'=>false,'error'=>'Password lama salah']); exit;
    }
